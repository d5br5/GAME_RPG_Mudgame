...
async function action(req, res) {
	...
	if (action === "move") {
		...
		if (sample < eventProb[0]) {
			// nothing
			event = "아무 일도 일어나지 않았다.";
		} else if (sample < eventProb[1]) {
			// item
		...
		if (sample > (int + str) / (int + str + player.str + player.int)) {
			...
			if (player.HP <= 0) {
				...
				if (player.items.length !== 0) {
					const removeItemIndex = Math.floor(
						Math.random() * player.items.length
					);
...
function _list(items) {
	...
	const x = Object.keys(res).reduce(
		(acc, cur) => acc + `${cur} : ${res[cur]}개\n`,
		""
	);
	return x;
}
